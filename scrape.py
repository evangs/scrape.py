...
Done:
  ...
  - submit forms with file upload (multipart form)
...
def fetch(url, data='', agent=None, referrer=None, charset=None, verbose=0,
    ...
    multipart = False
    if type(data) == MultiPartForm:
        multipart = True

    # Prepare the POST data.
    method = data and 'POST' or 'GET'
    if data and not isinstance(data, str) and not multipart: # Unicode not allowed here
        data = urlencode(data)
...
class Session:
    ...
    def submit(self, region, paramdict=None, url=None, redirects=10, 
               multipart_data=None, **params):
        ...
        multipart = False
        if multipart_data and type(multipart_data) == MultiPartForm:
            multipart = True
        ...
        if multipart:
            ...
            if method == 'post':
                return self.go(url, multipart_data, redirects)
            else:
